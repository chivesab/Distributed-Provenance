...
class NodeCluster(
    node_grpc.NodeReplicationServicer,
    client_grpc.StreamingServicer,
    sentinel_grpc.SentinelMonitoringServicer,
):
    jwt_token = None
    master_ip = None
    file_root_dir = "./data/"
    ...
    def login_or_register(self):
        ...
        with grpc.insecure_channel(self.gateway_ip) as gateway_channel:
            ...
            auth_response = stub.Login(
                gateway_pb2.Request(ip=NODE_IP, password=NODE_PASSWORD, type="NODE")
            )

            if auth_response.message != "SUCCESS":
                auth_response = stub.Register(
                    gateway_pb2.Request(ip=NODE_IP, password=NODE_PASSWORD, type="NODE")
                )

            message, token, master_ip = itemgetter("message", "token", "masterip")(
                auth_response
            )
    ...
    def file_to_create_replica_request(self, filename):
        with open(self.get_file_path(filename), "rb") as f:
            chunk = f.read(1000)
            while len(chunk) > 0:
                response = node_pb2.CreateReplicaRequest(
                    filename=filename, payload=chunk
                )
                chunk = f.read(1000)
                yield response
        ...
        for node_ip in node_ips:
            ...
            with grpc.insecure_channel(node_ip) as other_node_channel:
                try:
                    ...
                    replica_response = stub.CreateReplica(
                        self.file_to_create_replica_request(filename)
                    )
                    if replica_response.status != "SUCCESS":
                        raise Exception("Replication failed")
                    logging.info(
                        "Successfully replicate file [%s] to node [%s]"
                        % (filename, node_ip)
                    )
                except Exception as e:
                    logging.error(
                        "Failed to replicate file [%s] to node [%s]"
                        % (filename, node_ip)
                    )
                    error = str(e)
    ...
    def validate_token(self, client_ip, client_token):
        with grpc.insecure_channel(self.gateway_ip) as gateway_channel:
            ...
            validate_response = stub.ValidateToken(
                gateway_pb2.ValidateTokenRequest(
                    client_ip=client_ip, token=client_token
                )
            )
    ...
    def UploadFile(self, request_iterator, context):
        ...
        if validate_response.message == "VALID" or filename is not None:
            try:
                ...
                error = self.replicate_file(
                    request_iterator.filename,
                    self.get_node_ips_for_replication(request_iterator.filename),
                )
                if error == None:
    ...
    def DownloadFile(self, request, context):
        ...
        if validate_response.message == "VALID":
            if not self.find_files(request.filename, self.file_root_dir):
                return iter(
                    [
                        client_pb2.DownloadFileReply(
                            payload=None, error="File Not Exist!"
                        )
                    ]
                )
            ...
            try:
                ...
                return iter(
                    [client_pb2.DownloadFileReply(payload=None, error="IO Exception!")]
                )
        else:
            return iter(
                [client_pb2.DownloadFileReply(payload=None, error="Validation Failed!")]
            )
